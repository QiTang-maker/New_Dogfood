...
class CodeWriter:
    ...
    @Writer.add(Command.LS)
    def _write_statfs(self, cmd):
        return f'do_ls("{cmd.path_}")'

    @Writer.add(Command.WC)
    def _write_wc(self, cmd):
        return f'do_wc("{cmd.path_}")'

    @Writer.add(Command.TAC)
    def _write_tac(self, cmd):
        return f'do_tac({str(cmd.fd_)}, {cmd.buf_id_}, {cmd.size_}, "{cmd.path_}")'

    @Writer.add(Command.CP)
    def _write_cp(self, cmd):
        return f'do_cp("{cmd.old_path_}", "{cmd.new_path_}")'

    @Writer.add(Command.CHMOD)
    def _write_chmod(self, cmd):
        return f'do_chmod("{cmd.path_}")'

    @Writer.add(Command.FCHMOD)
    def _write_chmod(self, cmd):
        return f'do_fchmod("{cmd.path_}")'

    @Writer.add(Command.TREE)
    def _write_tree(self, cmd):
        return f'do_tree("{cmd.path_}")'

    @Writer.add(Command.LSTAT)
    def _write_lstat(self, cmd):
        return f'do_lstat("{cmd.path_}")'

    @Writer.add(Command.SPLIT)
    def _write_split(self, cmd):
        return f'do_split("{cmd.path_}", {cmd.lines_})'

    @Writer.add(Command.TMPWATCH)
    def _write_tmpwatch(self, cmd):
        return f'do_tmpwatch("{cmd.path_}")'

    @Writer.add(Command.FILE)
    def _write_file(self, cmd):
        return f'do_file("{cmd.path_}")'
