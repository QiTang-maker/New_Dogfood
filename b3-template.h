...
#include "/home/icse20/crashmonkey/code/tests/BaseTestCase.h"
#include "/home/icse20/crashmonkey/code/user_tools/api/workload.h"
#include "/home/icse20/crashmonkey/code/user_tools/api/actions.h"
    ...
    namespace tests {
        ...
        class Example: public BaseTestCase {
            ...
            private:
                ...
                int cp_impl(const char *old_path, const char *new_path);
                int split_impl(const char *path, int lines);
...
#define do_ls(path)\
    do { \
        char *real_path = (char*)patch_path(path); \
        char cmd[1024]; \
        snprintf(cmd, 1024, "ls %s", real_path); \
        system(cmd); \
    } while (0)
...
#define do_wc(path)\
    do { \
        char *real_path = (char*)patch_path(path); \
        char cmd[1024]; \
        snprintf(cmd, 1024, "wc -c %s", real_path); \
        system(cmd); \
    } while (0)

#define do_tac(fd, buf_id, size, path) \
    do { \
        if (read(fd, buffers[buf_id], size) < 0) { \
            return errno; \
        } \
        char *real_path = (char*)patch_path(path); \
        char cmd[1024]; \
        snprintf(cmd, 1024, "tac %s", real_path); \
        system(cmd); \
    } while (0)

namespace fs_testing {
    namespace tests {
        int Example::cp_impl(const char *old_path, const char *new_path) {
            int old_fd = cm_->CmOpen(old_path, O_RDWR);
            if (old_fd == -1) {
                return errno;
            }

            char *cp_buffer = (char*)align_alloc(SIZE_PER_BUF);
            if (!cp_buffer) {
		fprintf(stderr, "Malloc failure\n");
		exit(1);
	    }
	    memset(cp_buffer, 'a', SIZE_PER_BUF);
            
            int new_fd = open(new_path, O_RDWR|O_CREAT, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
            if (new_fd < 0) {
                close(new_fd);
                return errno;
            }

            int status = read(old_fd, cp_buffer, 20 * 4096);
	    if (status < 0) {
            return errno;
            }
            
            status = cm_->CmWrite(new_fd, cp_buffer, 20 *4096);
	    if (status < 0) {
	        return errno;
            }
...
#define do_cp(old_path, new_path) \
    do { \
        char *real_old_path = (char*)patch_path(old_path); \
        char *real_new_path = (char*)patch_path(new_path); \
        if (cp_impl(real_old_path, real_new_path) < 0) { \
            return errno; \
        } \
    } while (0)


#define do_chmod(path)\
    do { \
        char *real_path = (char*)patch_path(path); \
        char cmd[1024]; \
        snprintf(cmd, 1024, "chmod 777 %s", real_path); \
        system(cmd); \
    } while (0)
...
#define do_tree(path)\
    do { \
        char *real_path = (char*)patch_path(path); \
        char cmd[1024]; \
        snprintf(cmd, 1024, "tree %s", real_path); \
        system(cmd); \
    } while (0)

#define do_lstat(path)\
    do { \
        char *real_path = (char*)patch_path(path); \
        char cmd[1024]; \
        snprintf(cmd, 1024, "lstat %s", real_path); \
        system(cmd); \
    } while (0)

char* path_split(const char *prefix, const int lines) {
    char *file_name = (char*)malloc(lines / 10 + 1 + 1);
    sprintf(file_name, "%d", lines);
    int len = strlen(prefix) + strlen(file_name) + 3;
    char *buf = (char*)malloc(len);
    int ret = snprintf(buf, len, "%s-%s", prefix, file_name);
    if (ret == len) {
	free(buf);
	fprintf(stderr, "PATH_SPLIT failure\n");
	return NULL;
    } else {
	buf[ret] = '\0';
    }
...
namespace fs_testing {
    namespace tests {
        int Example::split_impl(const char *path, int lines) {
            for (int i = 0; i < lines; ++i) {
                char *split_path = path_split(path, i);
                char *real_path = (char*)patch_path(split_path);

		int fd = open(real_path, O_RDWR|O_CREAT, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH); 
		if (fd < 0) { 
		    close(fd); 
		    return errno;
		}
...
#define do_split(path, lines) \
    do { \
        if (split_impl(path, lines) < 0) { \
        return errno; \
        } \
    } while (0)

#define do_tmpwatch(path) \
    do { \
        char *real_path = (char*)patch_path(path); \
        char cmd[1024]; \
        snprintf(cmd, 1024, "tmpwatch 1 %s", real_path); \
        system(cmd); \
    } while (0)
...
#define do_file(path)\
    do { \
        char *real_path = (char*)patch_path(path); \
        char cmd[1024]; \
        snprintf(cmd, 1024, "file %s", real_path); \
        system(cmd); \
    } while (0)
