...
class Command:
    ...
    def all_types():
        return (
            ...
            Command.LS,         Command.WC,
            Command.TAC,        Command.CP,
            Command.CHMOD,      Command.FCHMOD,
            Command.TREE,       Command.LSTAT,
            Command.SPLIT,      Command.TMPWATCH,
            Command.FILE
        )
...
class Ls(Command):
    ...
    def __str__(self):
        return f'LS {self.path_}'

    def __deep_copy__(self, memo):
        return Ls(self.path_)

class Wc(Command):
    def __init__(self, path):
        super().__init__(Command.WC)
        self.path_ = path

    def __str__(self):
        return f'WC {self.path_}'

    def __deep_copy__(self, memo):
        return Wc(self.path_)

class Tac(Command):
    def __init__(self, fd, path):
        super().__init__(Command.TAC)
        self.fd_ = fd
        self.path_ = path
    ...
    def __str__(self):
        return f'TAC {str(self.fd_)} {self.path_}'

    def __deep_copy__(self, memo):
        return Tac(copy.deepcopy(self.fd_), self.path_)

class Cp(Command):
    def __init__(self, new_path, old_path):
        super().__init__(Command.CP)
        self.old_path_ = old_path
        self.new_path_ = new_path

    def __str__(self):
        return f'CP {self.old_path_} {self.new_path_}'

    def __deep_copy__(self, memo):
        return Cp(self.new_path_, self.old_path_)

class Chmod(Command):
    def __init__(self, path):
        super().__init__(Command.CHMOD)
        self.path_ = path

    def __str__(self):
        return f'CHMOD {self.path_}'

    def __deep_copy__(self, memo):
        return Chmod(self.path_)

class Fchmod(Command):
    def __init__(self, path):
        super().__init__(Command.FCHMOD)
        self.path_ = path

    def __str__(self):
        return f'FCHMOD {self.path_}'

    def __deep_copy__(self, memo):
        return Fchmod(self.path_)

class Tree(Command):
    def __init__(self, path):
        super().__init__(Command.TREE)
        self.path_ = path

    def __str__(self):
        return f'TREE {self.path_}'

    def __deep_copy__(self, memo):
        return Tree(self.path_)

class Lstat(Command):
    def __init__(self, path):
        super().__init__(Command.LSTAT)
        self.path_ = path

    def __str__(self):
        return f'LSTAT {self.path_}'

    def __deep_copy__(self, memo):
        return Lstat(self.path_)

class Split(Command):
    def __init__(self, path):
        super().__init__(Command.SPLIT)
        self.path_ = path

        self.lines_ = random.randint(1, 1 << 3)

    def __str__(self):
        return f'Split {self.path_}'

    def __deep_copy__(self, memo):
        return Split(self.path_)

class Tmpwatch(Command):
    def __init__(self, path):
        super().__init__(Command.TMPWATCH)
        self.path_ = path

    def __str__(self):
        return f'TMPWATCH {self.path_}'

    def __deep_copy__(self, memo):
        return Tmpwatch(self.path_)

class File(Command):
    def __init__(self, path):
        super().__init__(Command.FILE)
        self.path_ = path

    def __str__(self):
        return f'FILE {self.path_}'

    def __deep_copy__(self, memo):
        return File(self.path_)
