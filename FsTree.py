...
class FsTree:
    ...
    def _add_cp(self, new_path, old_path):
        #target_node = self._must_lookup_node(old_path)

        node = FileNode()
        self._add_node(new_path, node)

    def _add_split(self, path, lines):
        parent_path = M4.parent_dir(path)
        file_name = path.rsplit('/', 1)[1]
        for _ in range(lines):
            if parent_path != '/':
                split_path = parent_path + '/' + file_name + '-' + f'{_}'
            else:
                split_path = parent_path + file_name + '-' + f'{_}'
            
            node = FileNode()
            #print(split_path)
            self._add_node(split_path, node)
    ...
    @Taker.add(Command.LS)
    def _take_ls(self, cmd):
        pass

    @Taker.add(Command.WC)
    def _take_wc(self, cmd):
        pass

    @Taker.add(Command.TAC)
    def _take_tac(self, cmd):
        pass

    @Taker.add(Command.CP)
    def _take_cp(self, cmd):
        self._add_cp(cmd.new_path_, cmd.old_path_)
        pass

    @Taker.add(Command.CHMOD)
    def _take_chmod(self, cmd):
        pass

    @Taker.add(Command.FCHMOD)
    def _take_fchmod(self, cmd):
        pass

    @Taker.add(Command.TREE)
    def _take_Tree(self, cmd):
        pass

    @Taker.add(Command.LSTAT)
    def _take_lstat(self, cmd):
        pass

    @Taker.add(Command.SPLIT)
    def _take_Split(self, cmd):
        self._add_split(cmd.path_, cmd.lines_)

    @Taker.add(Command.TMPWATCH)
    def _take_Tmpwatch(self, cmd):
        self._del_subtree(cmd.path_)

    @Taker.add(Command.FILE)
    def _take_file(self, cmd):
        pass
